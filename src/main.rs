...
//! 
//! There will also be some minor differences between how escape sequences and control characters are handled, and file send/receive is not yet implemented (still deciding how best to do that). 
//! 
//! ## Usage
//! 
...
//! ### Runtime Control
//! 
//! When the application is running, some settings can be changed, this is done by entering command mode by pressing `Ctrl + [Escape Key]`, the escape key is determined by the `--escape / -e` flag at startup or is default to `a`. 
//! 
//! If you wish to send a `Ctrl` character into the serial port that is the same as the command key, pressing the same command key again will send that character to the serial port. 
//! 
//! Once command mode is entered, a command is given by pressing `Ctrl` and then a key mapped to a command, for picocom users, **all** of these commands are the same, with `Ctrl s + r + \` being absent. 
//! 
//! A confirmation message will be shown when the command has been executed and the program has excited command mode, all commands except `Ctrl b` (change baud-rate) which requires further input, will be executed and exit immediately. 
//! 
...
//! - `Ctrl c`
//!     Toggles local echo on and off. If on the application will print any and all characters typed to the terminal regardless of weather the serial device echoes them back. 
//! - `Ctrl v`
...
//! ## Development 
//! 
//! I have never written a terminal emulator before, and this has all been a big yak shave originating from being stuck with a Windows PC and wishing I could use picocom. I have been reliant upon Wikipedia, throwing bytes at other terminal emulators to see how they behave, and reading some really godawful C code. 
//! 
//! As a result there may be numerous bugs, and numerous gaps in my own understanding, but as of writing it seems to behave mostly like picocom under a Unix like environment and works pretty much the same in Windows. 
//! 
//! With that said, any bugs or general points raised would be very welcome, feel free to raise a bug, open a pull request, message me or however you wish. 
//! 
//! Current roadmap is:
//! 
//! * File send and receive
//!     * Picocom does this via invoking other standard unix applications, should we follow this lead (it would be fairly simple to reimplement them cross platform) or bundle the functionality within this application, this will make the interface slightly more different. 
//! * More rich escape sequence handling 
//!     * We currently handle a few different cursor controls and tabs, a cursory glance reveals no ANSI escape sequence parsers that would really work well with this application, a custom one shouldn't be too difficult. 
//! 
//! 
